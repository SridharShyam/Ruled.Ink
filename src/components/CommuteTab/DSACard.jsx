...
export default function DSACard() {
  ...
  const problem = leetcodeProblems[dayOfYear % leetcodeProblems.length];
    ...
    <div className="notebook-card p-6 flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-3">
        <div className="flex gap-2">
          <span className={`px-2 py-0.5 rounded-sm text-[9px] font-medium uppercase ${difficultyColors[problem.difficulty]}`}>
            {problem.difficulty}
          </span>
          <span className="px-2 py-0.5 bg-surface2 text-muted rounded-sm text-[9px] font-medium uppercase tracking-widest font-mono">
            {problem.topic}
          </span>
        </div>
      ...
      <a 
        ...
        className="mt-4 w-full bg-surface2 border border-border3 hover:border-accent2 py-3 rounded-md text-accent2 font-medium text-[13px] tracking-widest flex items-center justify-center gap-3 transition-all"
      >
