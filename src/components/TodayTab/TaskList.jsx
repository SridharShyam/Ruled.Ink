  ...
  return (
    <div className="flex flex-col">
      <form onSubmit={handleAdd} className="flex flex-col sm:flex-row gap-3 sm:gap-4 mb-8 px-4">
        ...
        />
        <button type="submit" className="bg-accent text-surface px-6 py-2 rounded-md text-[14px] font-medium hover:bg-accent/90 transition-all sm:w-auto w-full">
          ADD MISSION
        </button>
      </form>
        ...
        {tasks.length === 0 ? (
          <div className="py-20 flex flex-col items-center justify-center text-center px-4">
            ...
            <p className="text-[14px] text-muted font-body">
              No missions yet. Add your first one above.
            </p>
          </div>
        ) : (
          tasks.map(task => (
            <div key={task.id} className="flex items-center gap-4 py-2.5 border-b border-border group px-4 hover:bg-surface2 transition-colors">
              ...
              {task.type && (
                <span className={`px-2.5 py-1 rounded-[4px] text-[11px] font-mono font-medium uppercase tracking-wider ${tagStyles[task.type] || 'bg-surface2 text-muted'}`}>
                  {task.type}
                </span>
              )}
            </div>
